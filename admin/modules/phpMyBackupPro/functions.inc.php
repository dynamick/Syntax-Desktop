...
/*
 +--------------------------------------------------------------------------+
 | Modifyed for use into Syntax Desktop                                     |
 | 2011-02-16                                                               |
 | don't update!!!                                                          |
 +--------------------------------------------------------------------------+
 | phpMyBackupPro                                                           |
...
echo "
 ...
 <tr>\n";
  ...
  foreach($menu as $filename=>$title) {
    if ($filename!="login.php?logout=TRUE" && $filename!="HELP") {
      $class =  ($filename==$scriptname) ?  ' class="active"' : '';
      $menu  = "  <th{$class} width=\"16%\">\n   <a href=\"".$filename."\" accesskey=\"".$accesskeys[$filename];
      $menu .= "\" title=\"[access key = ".$accesskeys[$filename]."]\">";
      $menu .= PMBP_image_tag(substr($filename, 0, strpos($filename,".")).'.png', "","[accesskey = ".$accesskeys[$filename]."]");
      $menu .= $title."</a>\n  </th>\n";
      echo $menu;
    }
  }
...
function PMBP_print_footer_orig() {
    global $PMBP_SYS_VAR;
    global $PMBP_MU_CONF;
    $tabe_width='100%';
    echo "\n<!-- FOOTER -->
...
function PMBP_exec_sql($file,$con,$linespersession=false,$noFile=false) {
    ...
    while (($linenumber<$_GET["start"]+$linespersession || $query!="") && ($dumpline=gzgets($file,65536)))
    {
        ...
        $dumpline = preg_replace('/\r$/', '', $dumpline);
