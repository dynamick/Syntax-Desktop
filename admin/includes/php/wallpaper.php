...
*/
$font_file  = '../../images/font/sqr721e.ttf' ;
$filename = "../../".$_GET["img"];
$w= $_GET["width"];
$h= $_GET["height"];
$text = $synWebsiteTitle ;
$description="Content management system for $text website"; 
$credits="Powered By SyntaxDesktop ".$synVersion; 
$font_size  = 20 ;
...
/*
  ...
  ---------------------------------------------------------------------------
*/
@putenv('GDFONTPATH=' . realpath('../../images/font/') );
...
if(!$font_found){fatal_error('Error: The server is missing the specified font.') ;}
...
$box2 = @ImageTTFBBox($font_size_description,0,$font_file,$description) ;
$box3 = @ImageTTFBBox($font_size_description,0,$font_file,$credits) ;
...
//imagecopyresampled($image, $image_orig, 0, 0, 0, 0, $new_width, $new_height, $width, $height);
imagecopyresized($image, $image_orig, 0, 0, 0, 0, $new_width, $new_height, $width, $height);

//$image = imagecreatefromjpeg("blue.jpg") ;

if(!$image) {
  //fatal_error('Error in Image: The server could not create this heading image.');
}
if(!$box) {//fatal_error('Error in Box: The server could not create this heading image.');
} else {
  ...
  $font_color_shadow = @ImageColorAllocate($image,143,197,230) ;   
  //ImageTTFText($image,$font_size,0,-$box[0],abs($box[5]-$box[3])-$box[1], $font_color,$font_file,$text) ;
  ImageTTFText($image,$font_size,0,floor($w/2)-floor($box[4]/2)+1,floor($h/2)-floor($box[5]/2)+1, $font_color_shadow, $font_file,$text) ;
  ImageTTFText($image,$font_size,0,floor($w/2)-floor($box[4]/2),floor($h/2)-floor($box[5]/2), $font_color,$font_file,$text) ;
  ImageTTFText($image,$font_size_description,0,floor($w/2)-floor($box2[4]/2)+1,floor($h/2)-floor($box2[5]/2)+20, $font_color, $font_file,$description) ;
  ImageTTFText($image,$font_size_description,0,floor($w/2)-floor($box3[4]/2)+1,floor($h/2)-floor($box3[5]/2)+33, $font_color, $font_file,$credits) ;
  // set transparency
  if($transparent_background) ImageColorTransparent($image,$background_color) ;
}
...
function fatal_error($message)
{
    ...
    if(function_exists('ImageCreate'))
    {
        $width = ImageFontWidth(5) * strlen($message) + 20 ;
