...
function smarty_function_account($params, &$smarty) {
  ...
  $newsletter_active  = false;                        # set to true if listmessenger is installed      
	$t        					= multiTranslateDictionary(array('password_dimenticata', 'registrati_qui', 'inserisci_dati', 'email', 'password', 'invia', 'annulla', 'nome', 'cognome', 'ragione_sociale', 'indirizzo', 'citta', 'cap', 'provincia', 'cinque_caratteri', 'necessaria_registrazione', 'dati_personali', 'vostro_account', 'conferma_password', 'letto_accettato', 'letto_informativa', 'privacy', 'newsletter', 'registrazione_newsletter', 'campo_obbligatorio', 'email_non_valida', 'almeno_cinque_caratteri', 'password_sbagliate', 'dati_salvati', 'conferma_registrazione', 'account_attivo', 'effettua_login', 'utente_password_errati', 'autenticato', 'effettua_logout', 'dati_aggiornati', 'inserisci_dati', 'password_rigenerata', 'nuovi_dati_inviati', 'email_sconosciuta', 'indirizzo_non_presente', 'riprova'));
...
<?
  ...
  #¦-------------------------------------------------------------------------¦
  } elseif (($_GET["action"]=="reg")||($_GET["action"]=="update")) {
    
    if (($_GET["action"]=="update") && ($userid)) {
